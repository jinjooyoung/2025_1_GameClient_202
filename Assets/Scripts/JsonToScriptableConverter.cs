...
{
    public int? id;                 // int?는 Nullable<int>의 축약 표현 -> 선언하면 null 값도 가질 수 있는 정수형이 됨
...
public class JsonToScriptableConverter : EditorWindow
{
    ...
    private string outputFolder = "Assets/ScriptableObjects";       // 출력 SO 파일을 저장할 경로 값
    private bool createDatabase = true;                             // 데이터 베이스를 사용할 것인지에 대한 bool값
    ...
    private void ConvertJsonToItemScriptableObjects()     // JSON 파일을 ScriptableObject 파일로 변환시켜주는 함수
    {
        // 폴더 생성
        ...
        try
        {
            ...
            foreach(var rowData in rowDataList)
            {
                ...
                if (!rowData.id.HasValue && !string.IsNullOrEmpty(rowData.choiceText) && rowData.choiceNextId.HasValue)
                {
                    ...
                    // 이 선택이 바로 위에 있는 대화 (id 가 있는 방목)을 찾음
